...
// the only function that has any hardcoded table / col references is inferInitiativesFieldType
...
/**
 * Custom field type inference for Initiatives table
 * Extends the generic inference with Initiatives-specific field types
 * @param {string} fieldName - The field name
 * @returns {string} - The inferred field type
 */
function inferInitiativesFieldType(fieldName) {
    const name = fieldName.toLowerCase();
    
    // OKR field detection - custom business logic for Initiatives
    if (name.includes('okr') || name.includes('okr_id')) {
        return 'okr_lookup';
    }
    
    // Responsible field detection - custom business logic for Initiatives
    if (name.includes('responsible') || name.includes('owner') || name.includes('assignee')) {
        return 'responsible';
    }
    
    // Unit field detection - custom business logic for Initiatives
...
// by default get all the Initiatives from the database during initial load and put it in the memory
...
// function will display Initiatives in a table
// Source of data is the "global" variable allinitiativesjson

/**
 * Displays all Initiatives from the specified table in a data table
 * @param {string} [tableName='initiatives'] - Name of the table to display Initiatives from
 * @throws {Error} If fetching or displaying Initiatives fails
 */
async function displayallitems (tableName = 'initiatives') {
    try {
        ...
        // this is a case where we do not expect more than 5000 rows at the end of life
        const allinitiativesjson = await fetchAllRows(tableName);

        console.log('All Initiatives JSON from DB:', allinitiativesjson?.length || 0, 'Initiatives');

        if (!Array.isArray(allinitiativesjson) || allinitiativesjson.length === 0) {
...
/**
 * Displays filtered Initiatives from the specified table based on search criteria
 * not optimizing further because for more complex apps this may get complex
 * @param {string} [tableName='initiatives'] - Name of the table to search in
 * @throws {Error} If searching or displaying filtered Initiatives fails
 */
async function displayfiltereditems (tableName = 'initiatives') {
    try {
        ...
        if (!searchValue) {
            // If no search term, just display all Initiatives
            displayallitems(tableName);
            return;
        }
        
        // Use the generic search function from restcallsfordbdata.mjs
        const filteredinitiativesjson = await fetchFilteredRows(tableName, searchValue);

        console.log('Filtered JSON from DB:', filteredinitiativesjson?.length || 0, 'Initiatives');

        if (!Array.isArray(filteredinitiativesjson) || filteredinitiativesjson.length === 0) {
        ...
        console.error('Error displaying filtered Initiatives:', error);
        handleError(error, { scope: 'page' });
    }
}
...
async function saveRow() {
    try {
        ...
        if (editedItemPkValue === null || editedItemPkValue === undefined || editedItemPkValue === '') {
            ...
            console.log('Creating new Initiative with payload:', payload);
            await createRow(currentTable, payload);
            console.log('Initiative created successfully');
        } else {
            // Update existing Initiative
            console.log('Updating Initiative with ID:', editedItemPkValue, 'payload:', payload);
        ...
        console.error('Error adding/updating Initiative:', error);
...
async function prepareEditForm(rowIndex) {
    try {
        ...
        const { tableMeta, formFields } = await buildDynamicForm(currentTable, rowData, inferInitiativesFieldType);
...
async function deleterow(rowIndex) {
    try {
        ...
        console.log('Deleting Initiative with primary key:', pkValue);
...
/**
 * Handles form submission with validation for adding/editing Initiatives
...
async function handleFormSubmission(){
    try {
        console.log("Handling form submission for Initiative addition/editing");
        
        // Use the generic validation and submission function with custom field type inference
        await validateAndSubmitForm(currentFormFields, currentTableMeta, saveRow, 'modal', inferInitiativesFieldType);
    } catch (error) {
    ...
    try {
        ...
        if (addNewBtn) {
            addNewBtn.addEventListener('click', async () => {
                try {
                    editedItemPkValue = null;
                    const { tableMeta, formFields } = await buildDynamicForm(currentTable, null, inferInitiativesFieldType);
                    currentTableMeta = tableMeta;
                    currentFormFields = formFields;
                } catch (error) {
